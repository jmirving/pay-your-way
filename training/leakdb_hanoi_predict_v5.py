...
def compact_ranking(pred_xy, coords, candidates) -> list[dict]:
    return rank_nodes(pred_xy, coords, candidates)[:RANKING_DEPTH]


def main() -> None:
    parser = argparse.ArgumentParser(description="Run fixed Hanoi incident policy with four-hour localization refresh.")
    ...
    for sid in hold_ids:
        ...
        for spec, xy in zip(episode_specs, predicted_xy):
            ...
            snapshots_by_episode[episode_idx].append({
                ...
                "ranked_nodes": compact_ranking(xy, coords, candidates),
            })
    ...
    report = {
        "algorithm": "Pay Your Way LeakDB Hanoi pipeline v5 four-hour dynamic localization",
        "detector": {
        ...
        "localizer": {
            "family": "ExtraTrees coordinate localizer with all previously revealed Hanoi events",
            ...
            "stored_ranking_depth": RANKING_DEPTH,
            "total_holdout_snapshots": total_snapshots,
        },
        "holdout_scenario_count": len(hold_ids),
        "raw_sensor_count": len(features or []),
        "scikit_learn_version": sklearn_version,
        "label_boundary": "No Hanoi-005 holdout leak timing or node labels are opened by prediction.",
    }
    ...
    print(
        f"Hanoi v5 predicted {len(hold_ids)} sealed scenarios with "
        f"{sum(len(r['alert_episodes']) for r in results)} incidents and "
        f"{total_snapshots} localization snapshots"
    )
