...
def pipeline_ids(leakdb_dir: Path, number: int) -> list[int]:
    return sorted(freeze_ids(leakdb_dir / f"pipeline_{number:03d}_prediction_freeze.json"))


def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare frozen-policy pipeline replication benchmark.")
    ...
    parser.add_argument("--challenge-id", default="leakdb-net1-pipeline-003")
    ...
    pipeline_001 = pipeline_ids(leakdb_dir, 1)
    pipeline_002 = pipeline_ids(leakdb_dir, 2)
    all_burned = sorted(set(detection_burned).union(localization_burned, pipeline_001, pipeline_002))
    ...
    # Identical localizer training universe to pipeline 001/002.
    localizer_dev_ids = sorted(set(detection_burned).union(localization_burned))
    needed_dev_ids = sorted(detector_train_ids.union(localizer_dev_ids))

    burned_set = set(all_burned).union(range(1, 21))
    candidates = [sid for sid in range(101, 1001) if sid not in burned_set]
    if len(candidates) < args.holdout_count:
        raise RuntimeError("Not enough fresh scenarios remain for pipeline-v3 holdout")
    holdout_ids = sorted(secrets.SystemRandom().sample(candidates, args.holdout_count))

    print(f"Pipeline v3 replication: {len(needed_dev_ids)} development + {len(holdout_ids)} fresh holdout scenarios")
    ...
    for sid in localizer_dev_ids:
        for event in catalog.get(str(sid), []):
            if event["node_id"] in candidate_set:
                localizer_events.append({"event_id": f"{sid}:{event['event_index']}", "scenario_id": sid, **event})
    ...
    manifest = {
        ...
        "task": "frozen-policy end-to-end replication",
        "benchmark": "KIOS-LeakDB",
        "network": "Net1",
        "detector_policy": {"threshold": 0.7, "smooth_window": 3, "hold_steps": 8, "source": "pipeline_002 frozen calibration"},
        "detector_training_scenario_ids": sorted(detector_train_ids),
        "localizer": "identical pipeline-001/002 ExtraTrees coordinate localizer",
        "localization_development_event_count": len(localizer_events),
        "excluded_pipeline_001_count": len(pipeline_001),
        "excluded_pipeline_002_count": len(pipeline_002),
