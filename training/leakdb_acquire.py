...
def load_frames(ids: list[int], cache_dir: Path):
    """Load LeakDB through the DataFrame path.

    WaterBenchmarkHub 0.4.0 currently raises inside LeakDB.load_data when
    return_X_y=True because Net1.load is invoked as a class method. The
    DataFrame path does not use that broken conversion path and exposes the
    same pressure/flow data plus labels, which we immediately separate.
    """
...
def main() -> None:
    ...
    manifest = {
        ...
        "holdout_selection": "cryptographically random sample at workflow runtime",
